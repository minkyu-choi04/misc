...
dest_dir = '/home/choi574/datasets/ImageNet2012_myclass100_val100/val/'
...
for c in dirs:
    ...
    #print(len(fn_list))
    print(fn_list)
    img_sample = np.random.choice(np.array(fn_list), 1, replace=False)
    print(img_sample)

    source_dir_img = os.path.join(c, img_sample[0])
    class_dir = c.split('/')[-1]
    dest_dir_class = os.path.join(dest_dir, class_dir)
    dest_dir_img = os.path.join(dest_dir_class, img_sample[0])
