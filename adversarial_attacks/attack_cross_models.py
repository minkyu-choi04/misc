...
def attack_LinfPGD(args, model1_o, model2_o, eps_list, img_s_load=256+128, img_s_return=224+112, cudnn_backends=True):
    ...
    model1 = torch.nn.Sequential(normalize, model1_o).cuda()
    model1_o.eval()
    model1.eval()
    model2 = torch.nn.Sequential(normalize, model2_o).cuda()
    model2_o.eval()
    model2.eval()

    val_loader, _, _ = dl.load_imagenet_myclass100_for_AdvAttacks(img_s_load=img_s_load, img_s_return=img_s_return, server_type=args.server_type)
    print('\n\n=============================')
    ...
    #for eps in [0.001, 0.002, 0.003, 0.005, 0.01]:
        ...
        for batch_i, data in enumerate(val_loader):
            ...
            ###################### 4 steps ####################
            prediction_on_purturbed_img = model1(x_adv)
            ...
            prediction_on_purturbed_img = model2_o(x_adv_norm)
            #logging.info("argmax(y^) = {}".format(torch.max(prediction_on_purturbed_img, 1)))
            #logging.info("prediction of purturbed image: {}".format(prediction_on_purturbed_img))
            #print("argmax(y^) = {}".format(torch.max(prediction_on_purturbed_img, 1)))
            ####print("prediction of purturbed image: {}".format(prediction_on_purturbed_img))
