...
def attack_LinfSPSAAttack(args, model_o, eps_list, n_sample=8192, batch_size=32, img_s_load=256+128, img_s_return=224+112, cudnn_backends=True, 
    ...
    for eps in eps_list:
        ...
        attack = LinfSPSAAttack(model, eps=eps, delta=delta, lr=0.01, nb_iter=n_iter, nb_sample=n_sample, max_batch_size=max_batch_size)
        '''
        ...
        for batch_i, data in enumerate(val_loader):
            ...
            with torch.no_grad():
                ...
                return_dict = model_o(x_adv_norm)

            tf_adv_steps = []
            for step in range(1):
                pred = torch.squeeze(return_dict)
            ...
            for step in range(1):
                acc1, acc5 = misc.accuracy(torch.squeeze(return_dict), torch.squeeze(labels), topk=(1, 5))
