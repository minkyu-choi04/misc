...
def attack_LinfSPSAAttack(args, model_o, eps_list, n_sample=8192, n_steps=16, batch_size=32, img_s_load=256+128, img_s_return=224+112, cudnn_backends=True, n_iter=100, delta=0.01, es=None, compensation=0):
    '''
    ...
    #for eps in [0.001, 0.002, 0.003, 0.005, 0.01]:
        ...
        attack = LinfSPSAAttack(
                model, eps=eps, delta=delta, lr=0.01, nb_iter=n_iter, nb_sample=n_sample, max_batch_size=batch_size)
        ...
        for batch_i, data in enumerate(val_loader):
            ...
            x_norm = normalize(inputs)
            rd = model[1].module.inference(x_norm, n_steps=n_steps)
            pred = rd['pred']
            tf_clean_steps = []
            for step in range(n_steps):
            ...
            with torch.no_grad():
                ...
                return_dict = model[1].module.inference(x_adv_norm, n_steps=n_steps)#, isReturnAllStep=True)
            #logging.info("argmax(y^) = {}".format(torch.max(prediction_on_purturbed_img, 1)))
            #logging.info("prediction of purturbed image: {}".format(prediction_on_purturbed_img))
            #print("argmax(y^) = {}".format(torch.max(prediction_on_purturbed_img, 1)))
            ####print("prediction of purturbed image: {}".format(prediction_on_purturbed_img))
