...
def attack_LinfAPGD(args, model_o, eps_list, n_steps=16, batch_size=128, img_s_load=256+128, img_s_return=224+112, cudnn_backends=True, n_iter=100, es=None, 
        n_rand_init=1, eps_iter=1.0/2.0, isTargeted=False, loss='difference_logits_ratio'):
    '''
    ...
    model = PyTorchClassifier(model=model_o, clip_values=(0,1), input_shape=(3, img_s_return, img_s_return), nb_classes=100,
            preprocessing=(np.asarray(mean).reshape((3, 1, 1)), np.asarray(std).reshape((3, 1, 1))), 
            device_type='gpu', loss=criterion)
    ...
    for eps in eps_list:
        ...
        if eps != 0.0: 
            attack = AutoProjectedGradientDescent(
                    ...
                    norm = np.inf, 
                    eps = eps, 
                    eps_step = eps / eps_iter, 
                    max_iter = n_iter, 
                    targeted = isTargeted, 
                    nb_random_init = n_rand_init, 
        ...
        for batch_i, data in enumerate(val_loader):
            ...
            if eps != 0.0:              
                if isTargeted:
                    lb = np.random.randint(0, 100, batch_size)
                else:
                    lb = labels.numpy()
                x_adv = attack.generate(inputs.numpy(), lb)
            else:
                x_adv = inputs
            toc = time.time()
            ...
            with torch.no_grad():
                ...
                x_adv_norm = (torch.tensor(x_adv).cuda() - mean_t)/std_t
