...
def attack_LinfCW(args, model_o, eps_list, n_steps=16, batch_size=128, img_s_load=256+128, img_s_return=224+112, cudnn_backends=True, n_iter=100, es=None, 
        n_rand_init=1, eps_iter=1.0/2.0, learning_rate=0.01, ):
    '''
    ...
    for eps in eps_list:
        ...
        for batch_i, data in enumerate(val_loader):
            ...
            tic = time.time()
            if eps != 0.0:              
                x_adv = attack.generate(inputs.numpy(), labels.numpy())
            else:
                x_adv = inputs
            toc = time.time()
