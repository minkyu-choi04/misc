...
def attack_L2CWAttack(args, model_o, n_steps=16, batch_size=128, img_s_load=256+128, img_s_return=224+112, cudnn_backends=True, es=10,
        learning_rate=0.01, binary_search_steps=9, max_iterations=10000, initial_const=0.001, step_compensation=0):
    '''
    ...
    for batch_i, data in enumerate(val_loader):
        ...
        tf_clean_steps = []
        for step in range(n_steps):
            tf = misc.get_classification_TF(pred, labels)
            #print(tf)
            ss = torch.squeeze(tf)
        ...
        tic = time.time()
        x_adv, eps = attack.perturb(inputs, labels)
        toc = time.time()

        if batch_i == 0:
            perturb = eps.view(-1, 1)
        else:
            perturb = torch.cat((perturb, eps.view(-1, 1)), 0)
        ...
        ####print("prediction of purturbed image: {}".format(prediction_on_purturbed_img))
        tf_adv_steps = []
        for step in range(n_steps-step_compensation):
    ...
    np.save('tf_adv.npy', tf_adv.cpu().numpy())
    np.save('tf_clean.npy', tf_clean.cpu().numpy())
    np.save('perturb.npy', perturb.cpu().numpy())
