...
#### update 2022/02/14
# This function is based on  attack4s_test8s_batch_EOTCorrect_record_memEfficient_targeted.py
# This code adds some new features
#       1. Save GT and adv labels
# That is all for now. The base code is being used by other running program, so I made a new copy. 


def attack_LinfPGD_EOT(args, model_o, eps_list, n_steps=16, batch_size=128, img_s_load=256+128, img_s_return=224+112, cudnn_backends=True, 
        n_iter=5, eps_iter=3, es=None, compensation=0, n_EOT=1, isAvgSteps=False, 
        shuffle_test=False, save_imgs=False, isVal1000=True, save_more=False, path_advLabels=None):
    '''
    ...
    if isVal1000:
        ...
        val_loader = dl.load_imagenet_myclass100_val100_for_AdvAttacks(batch_size=batch_size, img_s_load=img_s_load, img_s_return=img_s_return, 
                server_type=args.server_type, drop_last=False)
    ...
    for eps in eps_list:
        ...
        for batch_i, data in enumerate(val_loader):
            ...
            startp = batch_i * batch_size
            endp = min((batch_i + 1) * batch_size, 100)
            labels = labels[startp:endp]
            ...
            #print(batch_i)
            if save_imgs and (batch_i==0 or batch_i==1):
                n_plots = min(24, int(args.batch_size))
        ...
        if save_more:
            ...
            np.save('img_clean_e'+str(int(eps*10000))+'.npy', clean_imgs_accum)
            np.save('fixs_e'+str(int(eps*10000))+'.npy', fixs_accum.cpu().numpy())
            np.save('labelsGT_e'+str(int(eps*10000))+'.npy', labelsGT_accum.cpu().numpy())
            np.save('labelsAdv_e'+str(int(eps*10000))+'.npy', labelsAdv_accum.cpu().numpy())
