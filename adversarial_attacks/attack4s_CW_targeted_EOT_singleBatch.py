...
from advertorch.attacks import CarliniWagnerL2Attack_EOT 
...
# This code implements EOT as extending batch dims. And this code only supports batch_size=1. After
#   EOT with n_EOT=n, total batch size will be n. 


def attack_L2CW_EOT(args, model_o, c_list, n_steps=16, img_s_load=256+128, img_s_return=224+112, cudnn_backends=False, 
    ...
    for cval in c_list:
        ...
        print('c-val: ', cval)
        attack = CarliniWagnerL2Attack_EOT(
        ...
        for batch_i, data in enumerate(val_loader):
            ...
            with torch.no_grad():
                inputs_duplicate = inputs.repeat([2, 1, 1, 1])
                # this duplication is used because batch=1 causes issue inside the model. 
                # it will be undone by selecting 0th batch to calculate TF. 
                #print(inputs_duplicate.size())
                inputs_norm = normalize(inputs_duplicate)
                ...
                for step in range(n_steps-compensation):
                    tf = misc.get_classification_TF(pred[step][0].unsqueeze(0), labels_gt)
                    #print(tf.size())
                    ss = torch.squeeze(tf)
            ...
            if cval != 0.0:              
                ...
                #print(torch.sum(x_adv[0]) == torch.sum(x_adv[1]))
                #print(torch.sum(x_adv[1]) == torch.sum(x_adv[6]))
                #print(torch.sum(x_adv[4]) == torch.sum(x_adv[14]))
                x_adv = x_adv[0].unsqueeze(0)
            else:
                x_adv = inputs
            toc = time.time()
            ...
            with torch.no_grad():
                ...
                x_adv_norm_duplicate = x_adv_norm.repeat(2, 1, 1, 1)
                return_dict = model_o(x_adv_norm_duplicate, n_steps=n_steps, isReturnAllStep=True)

            tf_adv_steps = []
            for step in range(n_steps-compensation):
                pred = torch.squeeze(return_dict['pred'][step])
                tf = misc.get_classification_TF(pred[0].unsqueeze(0), torch.squeeze(labels).unsqueeze(0))
                #print(tf)
                ss = torch.squeeze(tf)
            ...
            if batch_i == 0:
                ...
                if save_more:
                    x_adv_accum = x_adv
                    fixs_accum = torch.stack(return_dict['fixs_xy']).permute(1, 0, 2)[0, :, :].unsqueeze(0) # (b, step, 2)
                ...
                if save_more:
                    ...
                    fixs_accum = torch.cat((fixs_accum, torch.stack(return_dict['fixs_xy']).permute(1, 0, 2)[0,:,:].unsqueeze(0)), 0)
            ...
            for step in range(n_steps-compensation):
                ...
                acc1, acc5 = misc.accuracy(torch.squeeze(return_dict['pred'][step])[0].unsqueeze(0), torch.squeeze(ls).unsqueeze(0), topk=(1, 5))
        ...
        np.save('./data/perturb_linf_e'+str(int(cval*10000))+'.npy', linf_accum)
        np.save('./data/perturb_l1_e'+str(int(cval*10000))+'.npy', l1_accum)
        np.save('./data/perturb_l2_e'+str(int(cval*10000))+'.npy', l2_accum)
        if save_more:
